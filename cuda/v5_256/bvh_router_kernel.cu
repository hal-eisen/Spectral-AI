...
#define BVH_LEVELS        4       // Tree depth
#define BVH_LEAVES      256       // 4^3 leaf experts
#define BVH_NODES       341       // 1 + 4 + 16 + 64
#define BVH_LEAF_OFFSET  85       // 1 + 4 + 16 (index of first leaf)
...
// ============================================================================
// MEMORY LAYOUT for 256-expert variant (BVH_NODES=341):
//  Total constant-memory budget is 64 KB, so the two big arrays (portals = 16 KB,
//  snell_w = 85 KB) are promoted to __device__ global memory. L2 cache amortizes
//  the cost — every thread in a warp reads the same node's portal/snell row,
//  so accesses are highly coherent.
//
// Kept in __constant__ (fast broadcast reads):
//   c_centers: 341 * 3 * 4 =  4092 B
//   c_radii:   341 * 4     =  1364 B
//   c_snell_b: 341 * 4     =  1364 B
//   TOTAL                  =  6820 B  (well under 64 KB)
// ============================================================================

__constant__ float c_centers[BVH_NODES * 3];        // (341, 3) = 4092 B
__constant__ float c_radii[BVH_NODES];              // (341,)   = 1364 B
__device__   float c_portals[BVH_NODES * PORTAL_SIZE]; // (341, 12) = 16 KB in global
__device__   float c_snell_w[BVH_NODES * SPEC_DIM]; // (341, 64) = 85 KB in global
__constant__ float c_snell_b[BVH_NODES];            // (341,)   = 1364 B
